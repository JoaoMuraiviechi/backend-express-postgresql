...
export const getById = async (req: Request, res: Response) => {
  try {
    ...
    if (note.userId !== userId) return res.status(403).json({ error: "Acesso negado à nota de outro usuário." });
...
export const put = async (req: Request, res: Response) => {
  try {
    ...
    const note = await noteService.getNoteById(Number(id));
    if (!note) return res.status(404).json({ error: "Nota não encontrada." });
    if (note.userId !== userId) return res.status(403).json({ error: "Acesso negado." });

    const updated = await noteService.replaceNote(Number(id), { title, content, tags, pinned });
...
export const patch = async (req: Request, res: Response) => {
  try {
    ...
    const updated = await noteService.patchNote(Number(id), data);
...
export const remove = async (req: Request, res: Response) => {
    ...
    await noteService.deleteNoteById(Number(id));
